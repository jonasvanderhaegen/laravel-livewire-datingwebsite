<div
    x-data="{
        ...
        // 4) Set the initial state of your sun/moon elements
        setInitialState() {
            if (! this.darkMode) {
                ...
                document.querySelector('.theme-selector-moon').style.transform =
                    'translate(0%, -50%) translate(12px, 0px) rotate(70deg) scale(0.6)'
        ...
        init() {
            // a) Make sure the <html> has the correct class on page‐load/navigate
            document.documentElement.classList.toggle('dark', this.darkMode)

            // b) Prep the icons
            this.setInitialState()

            // c) Animate on every toggle
...
>
    ...
    {{-- Moon --}}
    <div
        class="theme-selector-moon absolute top-1/2 right-1/2"
        aria-hidden="true"
    >
    ...
    >
        <div class="relative h-full w-full">
            ...
            {{-- Right --}}
            <div
                class="absolute top-1/2 right-[0.45rem] h-[3px] w-[1.5px] -translate-y-1/2 rotate-[90deg] rounded-full bg-current transition duration-300"
            ></div>
            {{-- Bottom --}}
            <div
            ...
            {{-- Left --}}
            <div
                class="absolute top-1/2 left-[0.45rem] h-[3px] w-[1.5px] -translate-y-1/2 rotate-[90deg] rounded-full bg-current transition duration-300"
            ></div>
            {{-- Top Right --}}
            <div
                class="absolute top-[0.65rem] right-[0.65rem] h-[3px] w-[1.5px] rotate-[45deg] rounded-full bg-current transition duration-300"
            ></div>
            {{-- Top Left --}}
            <div
                class="absolute top-[0.65rem] left-[0.65rem] h-[3px] w-[1.5px] rotate-[-45deg] rounded-full bg-current transition duration-300"
            ></div>

            {{-- Bottom Right --}}
            <div
                class="absolute right-[0.65rem] bottom-[0.65rem] h-[3px] w-[1.5px] rotate-[-45deg] rounded-full bg-current transition duration-300"
            ></div>
            {{-- Bottom Left --}}
            <div
